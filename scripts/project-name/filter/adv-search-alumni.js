    ...
    [
        ...
        function ($repository, $log) {
            return {
                restrict: 'AE',
                replace: true,
                template: kendo.template($("#advSearchAlumni").html(), { useWithBlock: false }),
                controller: ['$scope', '$attrs', function ($scope, $attrs) {
                    ...
                    $scope.FilterBy = {
                        ...
                        Position: null,
                        JobLevel: null,
                        SeparatedDateFrom: null,
                        SeparatedDateTo: null
                    }
                    ...
                    $scope.filterApp = function () {
                        ...
                        $log.log($scope.FilterBy.SeparatedDateFrom);
                        $log.log($scope.FilterBy.SeparatedDateTo);

                        var filter = { logic: 'and', filters: [] };

                        if ($scope.FilterBy.SeparatedDateFrom !== null) { //|| $scope.FilterBy.AppliedDateFrom !== undefined
                            filter.filters.push({
                                field: 'separateddatefrom',
                                operator: 'startswith',
                                value: kendo.toString($scope.FilterBy.SeparatedDateFrom, 'yyyy-MM-dd')
                            });
                        }
                        if ($scope.FilterBy.SeparatedDateTo !== null) { //|| $scope.FilterBy.AppliedDateTo !== undefined
                            filter.filters.push({
                                field: 'separateddateto',
                                operator: 'endswith',
                                value: kendo.toString($scope.FilterBy.SeparatedDateTo, "yyyy-MM-dd")
                            });
                        }
                        ...
                        $('#grdAlumni').data('kendoGrid').dataSource.filter(filter);
                    };
                }]
            }
        }
